...
export function Table<T extends ScalarDict>({ data, columns, padding = 1, cell = Cell, header = Header, skeleton = Skeleton }: TableProps<T>) {
	const allColumns: (keyof T)[] = columns || (Array.from(new Set(data.flatMap((row) => Object.keys(row)))) as (keyof T)[]);
	...
	const headings = Object.fromEntries(allColumns.map((col) => [col, col])) as Partial<T>;
...
function row<T extends ScalarDict>(config: RowConfig): (props: RowProps<T>) => React.JSX.Element {
	...
	return ({ key, data, columns }) => (
		<Box flexDirection="row" key={key}>
			...
			{...intersperse(
				(i) => <skeleton.component key={`${key}-sep-${i}`}>{skeleton.cross}</skeleton.component>,
